...
Includes Elliott, Gaussian, GCU, Hexpo, ISigmoid, LiSHT, PSigmoid, PTanh,
PTELU, ReSech, SmeLU, Snake, SRS, and STanh.
"""
...
class Elliott(Layer):
    ...
    def call(self, inputs):
        """Apply the activation function to inputs."""
        inputs = tf.cast(inputs, "float32")
        y = 0.5 * inputs / (1.0 + tf.abs(inputs)) + 0.5
...
class Hexpo(Layer):
    ...
    Notes:
        Parameter names differ from the paper: a->alpha, b->beta, c->gamma, d->delta.
    """
    ...
    def call(self, inputs):
        ...
        y = tf.where(
            inputs >= 0.0,
            -self.alpha * (tf.exp(-inputs / (self.beta + 1e-7)) - 1.0),
            self.gamma * (tf.exp(inputs / (self.delta + 1e-7)) - 1.0),
        )
...
class ISigmoid(Layer):
    ...
    Notes:
        Parameter 'a' in the paper referenced above is called 'beta' in this layer.
    """
    ...
    def call(self, inputs):
        ...
        y = tf.where(
            inputs >= self.beta,
            (self.alpha * (inputs - self.beta)) + tf.sigmoid(self.beta),
            tf.where(
                inputs <= -self.beta,
                (self.alpha * (inputs + self.beta)) + tf.sigmoid(self.beta),
                tf.sigmoid(inputs),
...
class PTanh(Layer):
    ...
    def call(self, inputs):
        """Apply the activation function to inputs."""
        inputs = tf.cast(inputs, "float32")
        y = tf.where(inputs >= 0.0, tf.tanh(inputs), self.alpha * tf.tanh(inputs))
...
class PTELU(Layer):
    ...
    def call(self, inputs):
        """Apply the activation function to inputs."""
        inputs = tf.cast(inputs, "float32")
        y = tf.where(inputs >= 0.0, inputs, self.alpha * tf.tanh(self.beta * inputs))
...
class SmeLU(Layer):
    ...
    def build(self, input_shape):
        ...
        self.beta = self.add_weight(
            ...
            regularizer=self.beta_regularizer,
            constraint=self.beta_constraint,
        )  # Learnable parameter (see Eq. 7 in the linked paper above)
    ...
    def call(self, inputs):
        ...
        y = tf.where(
            ...
            0.0,  # Condition 1
            tf.where(
                tf.abs(inputs) <= self.beta,
                tf.square(inputs + self.beta) / (4.0 * self.beta),  # Condition 2
                inputs,
            ),
        )  # Condition 3 (if x >= beta)
...
class Snake(Layer):
    ...
    def call(self, inputs):
        ...
        inputs = tf.cast(inputs, "float32")
        y = inputs + ((1.0 / (self.alpha + 1e-7)) * tf.square(tf.sin(self.alpha * inputs)))
