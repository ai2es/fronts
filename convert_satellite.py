...
def _interpolate(band_num, ds_goes16, ds_goes17, lon_g16, lat_g16, lon_g17, lat_g17, new_lat_g16, new_lon_g16, new_lat_g17, new_lon_g17, return_dict):
    ...
    return_dict["CMI_C%02d" % band_num] = merged_data
...
if __name__ == '__main__':
    ...
    if args['multiprocessing']:
        ...
        for band_num in band_nums:
            p = multiprocessing.Process(target=_interpolate,
                args=(band_num, ds_goes16, ds_goes17, lon_g16, lat_g16, lon_g17, lat_g17, new_lat_g16, new_lon_g16, new_lat_g17, new_lon_g17, return_dict))
            processes.append(p)
            p.start()
        
        for i, t in enumerate(processes):
            t.join()
            band_str = "CMI_C%02d" % int(band_nums[i])
            ds_merged[band_str] = (('longitude', 'latitude'), return_dict[band_str].astype(np.float32))
