...
if __name__ == "__main__":
    ...
    if args['patience'] is None:
        patience = int(len(training_dataset) / (train_valid_batch_size[0] * train_valid_steps[0])) + 1
    ...
    training_dataset = training_dataset.batch(train_valid_batch_size[0], drop_remainder=True, num_parallel_calls=args['num_parallel_calls'])
    ...
    early_stopping = EarlyStopping('val_loss', patience=patience, verbose=1)  # EarlyStopping: stops training early if a metric does not improve after a specified number of epochs (patience)
