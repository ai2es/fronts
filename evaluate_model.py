...
def generate_predictions(model_number: int, model_dir: str, variables_netcdf_indir: str, prediction_method: str, domain: str = 'full',
    ...
    domain_extent = settings.DEFAULT_DOMAIN_EXTENTS[domain]
...
def prediction_plot(model_number: int, model_dir: str, fronts_netcdf_dir: str, timestep: str, domain: str, domain_images: tuple | list,
    ...
    if not isinstance(timestep, (list, tuple)):
        raise TypeError(f"timestep must be a list or tuple, received {type(timestep)}")
    elif len(timestep) != 4:
        raise TypeError(f"timestep must be length 4, received length {len(timestep)}")
    if not isinstance(domain, str):
        raise TypeError(f"domain must be a string, received {type(domain)}")
    ...
    DEFAULT_COLORBAR_POSITION = {'conus': 0.71, 'full': 0.78, 'global': 0.74}
    ...
    fig, ax = plt.subplots(1, 1, figsize=(20, 8), subplot_kw={'projection': ccrs.LambertConformal(central_longitude=250)})
