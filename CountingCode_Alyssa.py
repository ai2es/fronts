 ...
 #                print(lats,lons)
        ...
        for i in range(0,len(types)):
            ...
            for group in groups:
                ...
                frequency[group[1].yit.values[0],group[1].xit.values[0]] += np.where(len(group[1]['Front Number'])>=1,1,0) 
...
#and xarray used to open the multiple nc files:
    xmls.to_netcdf('FrontalCounts_' + '%04d%02d%02d'%(args.year, args.month, args.day) + '.nc')
